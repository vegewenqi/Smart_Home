...
class SmartHome_simplified(Env, ABC):
    ...
    def conti_model_sym(
        self, state: csd.SX, action: csd.SX, uncertainty: csd.SX, theta_model: csd.SX
    ):
        # states
        T_in = state[0]
        T_g = state[1]
        ...
        # ODEs
        d_T_in = (1 / self.C_in) * (
            theta_model[0] * self.k_w_out * self.k_w_in * (T_out - T_in)
            + theta_model[1] * self.k_g_in * (T_g - T_in)
        ) + theta_model[6]
        d_T_g = (1 / self.C_g) * (
            theta_model[2] * self.k_g_in * (T_in - T_g)
    ...
    def discrete_model_mpc(
        ...
        k2 = self.conti_model_sym(
            state + 0.5 * self.dt * k1, action, uncertainty, theta_model
        )
        k3 = self.conti_model_sym(
            state + 0.5 * self.dt * k2, action, uncertainty, theta_model
        )
        k4 = self.conti_model_sym(
            state + self.dt * k3, action, uncertainty, theta_model
        )
    ...
    def reset(self):
        ...
        self.state = self.state.clip(self.state_space.low, self.state_space.high)
        self.t = 0
        self.obs = extract_state(self.state)
    ...
    def step(self, action):
        ...
        self.t += 1
        self.obs = extract_state(self.state)
        return self.state, self.obs, rew, done
...
if __name__ == "__main__":
    ...
    with open("../../Settings/other/smarthome_rl_mpc_lstd.json", "r") as f:
        params = json.load(f)
        print(params)
    a = SmartHome_simplified(params["env"], params["env_params"], 1)
