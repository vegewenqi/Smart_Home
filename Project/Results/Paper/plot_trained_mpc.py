...
def plot_data(data1, data2, state_dim, action_dim, n_step):
    ...
    for i in range(3):
        for j in range(2):
            if not ai == 5:
                ...
                ax[i, j].plot(time, action2[ai, :], label=r'$\theta_{\star}$')
    ...
    ax[0].plot(time, rollout_return1, color='r', linestyle='dashed', label=r'$\theta_0$')
    ax[0].fill_between(x=time, y1=rollout_return_spo1, label='spo', alpha=0.3)
    ax[0].fill_between(x=time, y1=rollout_return_temp1, label='temp', alpha=0.3)
    ax[0].legend()
    ax[1].plot(time, rollout_return2, color='r', linestyle='dashed', label=r'$\theta_{\star}$')
    ax[1].fill_between(x=time, y1=rollout_return_spo2, label='spo', alpha=0.3)
    ax[1].fill_between(x=time, y1=rollout_return_temp2, label='temp', alpha=0.3)
    ax[1].legend()
