...
def plot_eval_perf(results_dir, params, eval_returns):
    print(params)
    print(eval_returns)
...
def rollout_sample(env, agent, replay_buffer, n_step, mode="train"):
    ...
    for _ in tqdm_context(range(n_step), desc="Episode", pos=1):
        ...
        next_obs, next_state, reward, done = env.step(action)
        if mode == "train" or mode == "mpc":
            replay_buffer.push(state, obs, action, reward, next_state, next_obs, done, add_info)
            # print([state, action])
            # print(reward)
        if mode == "final":
            print([state, action])
