...
class Custom_QP_formulation:
    def __init__(self, env, opt_horizon, gamma=1.0, th_param="custom", upper_tri=False):
        ...
        self.theta = csd.vertcat(self.theta_model, self.theta_in, self.theta_en)
    ...
    def opt_formulation(self):
        ...
        for i in range(self.N - 1):
            ...
            hx.append((20.0 - self.Sigma[0, i]) - (self.X[2, i]))
            hx.append((self.X[2, i]) - (60.0 + self.Sigma[0, i]))
            # 1 + sigma_e < e + theta_e< 4 + sigma_e
            hx.append((1.0 - self.Sigma[1, i]) - (self.X[3, i]))
            hx.append((self.X[3, i]) - (4.0 + self.Sigma[1, i]))
        ...
        hx.append((20.0 - self.Sigma[0, self.N - 1]) - (self.X[2, self.N - 1]))
        hx.append((self.X[2, self.N - 1]) - (60.0 + self.Sigma[0, self.N - 1]))
        hx.append((1.0 - self.Sigma[1, self.N - 1]) - (self.X[3, self.N - 1]))
        hx.append((self.X[3, self.N - 1]) - (4.0 + self.Sigma[1, self.N - 1]))
        for j in range(self.sigma_dim):
    ...
    def stage_cost_fn(self):
        ...
        l_tem = self.theta_in[0] * (self.x[0] - 23.0) * (self.x[0] - 27.0)
