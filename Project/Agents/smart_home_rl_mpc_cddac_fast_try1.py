...
class Custom_QP_formulation:
    ...
    def opt_formulation(self):
        J = 0
        W = np.ones((1, self.sigma_dim)) * [1, 1]
        g = []
        hx = []
        hu = []
        hsg = []
        ...
        for i in range(self.N - 1):
            ...
            hx.append((20 - self.Sigma[0, i]) - (self.X[2, i] + self.theta_t))
            hx.append((self.X[2, i] - self.theta_t) - (60 + self.Sigma[0, i]))
        ...
        hx.append((20 - self.Sigma[0, self.N - 1]) - (self.X[2, self.N - 1] + self.theta_t))
        hx.append((self.X[2, self.N - 1] - self.theta_t) - (60 + self.Sigma[0, self.N - 1]))
        hx.append((1 - self.Sigma[1, self.N - 1]) - (self.X[3, self.N - 1] + self.theta_e))
        hx.append((self.X[3, self.N - 1] - self.theta_e) - (4 + self.Sigma[1, self.N - 1]))
        for _ in range(self.sigma_dim):
    ...
    def stage_cost_fn(self):
        ...
        l_tem = self.theta_in[0] * (self.x[0] - 23 - self.theta_in[1]) * (self.x[0] - 27 - self.theta_in[2])
